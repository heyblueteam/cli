...
var listCmd = &cobra.Command{
	...
	Long:  "List user profiles for a workspace or company-wide.",
	Example: `  blue users list -s
  blue users list --workspace <id> --search "john"
  blue users list --first 100 --company <id>`,
	RunE: runList,
}
...
func init() {
	listCmd.Flags().StringVarP(&listWorkspace, "workspace", "w", "", "Workspace ID to get users from (if not specified, attempts company-wide)")
	listCmd.Flags().StringVar(&listCompany, "company", "", "Company ID (optional, uses default from config if not specified)")
	listCmd.Flags().StringVar(&listSearch, "search", "", "Search users by name or email")
	listCmd.Flags().IntVar(&listFirst, "first", 50, "Number of users to fetch")
	listCmd.Flags().BoolVarP(&listSimple, "simple", "s", false, "Show only basic user info")
}
...
func runList(cmd *cobra.Command, args []string) error {
	...
	targetCompanyID := listCompany
	if targetCompanyID == "" {
		targetCompanyID = client.GetCompanyID()
	}

	cInfo := getCompanyInfo(client, targetCompanyID)
	...
	if listWorkspace != "" {
		pInfo = getProjectInfo(client, listWorkspace)
		projectDisplay := getProjectDisplayName(pInfo)
		fmt.Printf("Fetching users from workspace %s...\n", projectDisplay)
		users, totalCount, err2 = getProjectUsers(client, listWorkspace, listFirst, listSearch)
	} else {
		companyDisplay := getCompanyDisplayName(cInfo)
		fmt.Printf("Fetching company-wide users (%s)...\n", companyDisplay)
		users, totalCount, err2 = getCompanyUsers(client, targetCompanyID, listFirst, listSearch)
	}
	...
	for _, user := range users {
		if listSearch != "" {
			searchTerm := strings.ToLower(listSearch)
	...
	if listSimple {
		...
		for _, user := range filteredUsers {
			...
			fmt.Printf("%-25s %-25s %-35s\n",
				user.ID,
				common.TruncateString(name, 25),
				common.TruncateString(user.Email, 35))
		}
	} else {
	...
	if listWorkspace != "" {
		projectDisplay := getProjectDisplayName(pInfo)
		fmt.Printf(" from workspace %s", projectDisplay)
	} else {
		companyDisplay := getCompanyDisplayName(cInfo)
...
// getCompanyDisplayName formats company information for display
