---
title: Records
description: Create, read, update, and organize records - the core unit of work in Blue, modeled as Todo objects in the API.
icon: SquareCheck
order: 0
---

Records are the core unit of work in Blue: the cards you create in a list, assign to people, tag, date, and fill with custom field values. In the API a record is a `Todo` object, and it lives inside a list (`TodoList`) within a workspace (`Project`).

This section covers creating records and every operation you reach for next - reading, updating, moving, completing, and organizing them. The most common starting point, `createTodo`, is documented in full below.
...
The `X-Bloo-Project-ID` header scopes record operations to one workspace. Company and project headers accept either an ID or a slug. Header names are case-insensitive.

## Operations

| Operation                                              | GraphQL                         | Description                                                                                                          |
| ------------------------------------------------------ | ------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Create a record                                        | `createTodo` mutation           | Add a record to a list, with optional tags, assignees, dates, checklists, and custom field values. Documented below. |
| [List records](/api/records/list-records)              | `todoQueries { todos }` query   | Retrieve and filter records in a workspace.                                                                          |
| [Update a record](/api/records/update-record)          | `updateTodo` mutation           | Change a record's title, dates, or other fields.                                                                     |
| [Toggle completion](/api/records/toggle-record-status) | `changeTodoDoneStatus` mutation | Mark a record done or not done.                                                                                      |
| [Move a record](/api/records/move-record-list)         | `updateTodo` mutation           | Move a record to a different list.                                                                                   |
| [Copy a record](/api/records/copy-record)              | `copyTodo` mutation             | Duplicate an existing record.                                                                                        |
| [Assignees](/api/records/assignees)                    | `updateTodo` mutation           | Assign or unassign users.                                                                                            |
| [Tags](/api/records/tags)                              | `setTodoTags` mutation          | Attach or detach tags.                                                                                               |
| [Comments](/api/records/add-comment)                   | `createComment` mutation        | Add a comment to a record.                                                                                           |
| [Dependencies](/api/records/dependencies)              | `createTodoDependency` mutation | Link records as blockers or dependents.                                                                              |
| [Recurring records](/api/records/recurring-records)    | `updateTodo` mutation           | Configure a repeating schedule.                                                                                      |
| [Archive a record](/api/records/archive-record)        | `archiveTodo` mutation          | Archive a record (and `unarchiveTodo` to restore it), hiding it from active lists while keeping its data.            |

Two lifecycle mutations don't yet have dedicated pages but round out the record surface:

| Operation           | GraphQL                                                | Description                                                                      |
| ------------------- | ------------------------------------------------------ | -------------------------------------------------------------------------------- |
| Delete a record     | `deleteTodo(input: DeleteTodoInput!): MutationResult!` | Permanently delete a record. `DeleteTodoInput` takes a single `todoId: String!`. |
| Bulk-update records | `updateTodos(input: UpdateTodosInput!): Boolean!`      | Apply the same change to many records at once.                                   |

## Create a record

Use the `createTodo` mutation to add a record to a list. The only required input is a `title`; everything else - the target list, dates, assignees, tags, checklists, and custom field values - is optional and can be set in the same call.
...
mutation CreateRecord {
  createTodo(input: { title: "Draft launch plan" }) {
    id
    title
    position
  }
...
| `placement`    | `CreateTodoInputPlacement`           | No       | Where to insert the record when `position` is not set. Defaults to the top of the list.  |
...
| `customFields` | `[CreateTodoInputCustomField]`       | No       | Initial custom field values.                                                             |
| `tags`         | `[CreateTodoTagInput!]`              | No       | Tags to attach.                                                                          |
...
`createTodo` returns the created `Todo`.
...
The mutation returns the `Todo` object. Common fields to select:
...
| Code                                | When                                                                                                                                      |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `PROJECT_NOT_FOUND`                 | No workspace context - the `X-Bloo-Project-ID` header is missing or invalid.                                                              |
...
- **Tags.** A `CreateTodoTagInput` with a `title` reuses an existing tag of the same title and color, or creates a new one (default color `#4a9fff`). A `title` with no match creates the tag.
