...
| Auto-generated chart | `Chart.metadata` (`ChartMetadata`) | A chart driven by a `barChart` (xAxis/yAxis) or `pieChart` (groupBy/value) spec.                                   |
...
| Page                                                                       | What it covers                                                                                                                                                                                                                 |
| -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [Create and manage charts](/api/charts/manage-charts)                      | The chart lifecycle: `createChart` (with optional inline `chartSegments` or `barChart`/`pieChart` metadata), `editChart` to rename/reposition/restyle, `copyChart` to duplicate with UID-remapped formulas, and `deleteChart`. |
...
| [Query and subscribe to charts](/api/charts/query-charts)                  | Read one chart with `chart(id:)` or list a dashboard's charts with `charts(filter:)`, including the `todoFilter` recalculation path, and `subscribeToChart` for real-time create/update/delete events.                         |
