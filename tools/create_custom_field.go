...
func RunCreateCustomField(args []string) error {
	fs := flag.NewFlagSet("create-custom-field", flag.ExitOnError)
	name := fs.String("name", "", "Custom field name (required)")
	fieldType := fs.String("type", "", "Custom field type (required)")
	projectID := fs.String("project", "", "Project ID (required for project-level custom fields)")
	description := fs.String("description", "", "Custom field description")
	options := fs.String("options", "", "Options for SELECT fields (format: 'value1:color1,value2:color2')")
	buttonType := fs.String("button-type", "", "Button type for BUTTON field type")
	buttonConfirmText := fs.String("button-confirm-text", "", "Button confirmation text")
	currencyFieldID := fs.String("currency-field-id", "", "Currency field ID for CURRENCY_CONVERSION type")
	conversionDate := fs.String("conversion-date", "", "Conversion date")
	conversionDateType := fs.String("conversion-date-type", "", "Conversion date type")
	min := fs.Float64("min", 0, "Minimum value for NUMBER type")
	max := fs.Float64("max", 0, "Maximum value for NUMBER type")
	currency := fs.String("currency", "USD", "Currency code")
	prefix := fs.String("prefix", "", "Field prefix")
	isDueDate := fs.Bool("is-due-date", false, "Whether this field represents a due date")
	timeDurationDisplay := fs.String("time-duration-display", "", "Time duration display type")
	timeDurationTargetTime := fs.Float64("time-duration-target", 0, "Time duration target time")
	referenceProjectID := fs.String("reference-project", "", "Reference project ID for REFERENCE type")
	referenceMultiple := fs.Bool("reference-multiple", false, "Allow multiple references")
	useSequenceUniqueID := fs.Bool("use-sequence", false, "Use sequence unique ID")
	sequenceDigits := fs.Int("sequence-digits", 6, "Number of digits in sequence")
	sequenceStartingNumber := fs.Int("sequence-start", 1, "Starting number for sequence")
	...
	if !validType {
		return fmt.Errorf("invalid field type '%s'. Use -list flag to see available types", *fieldType)
	}
